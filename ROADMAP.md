...
The cross-link web keeps compounding (agent-memory ↔ agent-loop ↔ tool-schema-design ↔ repo-level-agent-instructions ↔ claude-code-hooks; evaluation ↔ agent-loop ↔ testing-patterns ↔ agent-memory; testing-patterns ↔ error-handling-boundaries ↔ agent-legible-code ↔ layering-and-hexagonal; color-scheme-patterns ↔ design-tokens-and-theming ↔ avoiding-the-ai-generated-look ↔ subject-driven-identity; interaction-states ↔ error-handling-boundaries ↔ avoiding-the-ai-generated-look; layering-and-hexagonal ↔ service-boundaries) — new entries should keep looking for a genuine link rather than restating a neighboring entry's point.
...
1. `architecture-patterns/`, `coding-patterns/`, and `tools-and-skills/` sit at 3 — `agentic-ai/` (4) and `ux-ui-design/` (5) have pulled ahead. Next passes should even these back up.
2. Candidate topics not yet covered: MCP server integration (`tools-and-skills/`), functional patterns or classic design patterns worth keeping (`coding-patterns/`), CQRS/event sourcing or frontend architecture (`architecture-patterns/`).
